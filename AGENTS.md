...
- Long-form documents (guides, research, specs) → the wiki at write time:
  `write_memory(project: "harness-wiki", …)`; keep the distilled facts in a memory note with
  a `documented_in` relation. `status:` only marks retirement
  (`deprecated`/`superseded`/`historical`/`resolved`); current facts carry none.

**2. Wiki (`wiki/`) — long-form reference.** Search on demand (`project: "harness-wiki"`);
...
(`project: "harness-design"`). Reference only: prototype code under `design/` is never
...
"harness-code"`. Grep is the fallback for unindexed languages.
...
`search_memories(project: "harness-templates")`, read the `manifest.md` + its `reference/`
