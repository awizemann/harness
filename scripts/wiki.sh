#!/usr/bin/env bash
#
# Harness wiki helper — wraps the local .wiki-worktree/ clone with a secret-scan.
#
# Usage:
...
#
# Bootstrap (one-time):
#   1. In GitHub repo Settings → Features → Wikis, enable Wikis.
#   2. Visit https://github.com/awizemann/harness/wiki and save any first page
#      via the UI (this is what creates the underlying .wiki.git repo).
#   3. From repo root:
#        git clone https://github.com/awizemann/harness.wiki.git .wiki-worktree
#
...
WIKI_REMOTE="https://github.com/awizemann/harness.wiki.git"
...
cmd_push() {
  ...
  log "Pushed. Verify at https://github.com/awizemann/harness/wiki"
}
