...
Services live under `Harness/Services/`. They expose a protocol per [`../standards/01-architecture.md §6`](https://github.com/awizemann/harness/blob/main/standards/01-architecture.md) and are injected (never accessed as singletons, with the rare `ToolLocator` exception).
...
- Subprocess invocation goes through `ProcessRunner` (per [`../standards/03-subprocess-and-filesystem.md`](https://github.com/awizemann/harness/blob/main/standards/03-subprocess-and-filesystem.md)).
...
**Always** add a row to [Core-Services](Core-Services):

| Status | Service | File | Isolation | Purpose |

If the service is non-trivial (>200 lines, multiple responsibilities, complex CLI surface, or has subtle behavioral guarantees), give it its own deep-dive page — copy the shape of [Simulator-Driver](Simulator-Driver).

### 7. Audit

Run [`../standards/AUDIT_CHECKLIST.md`](https://github.com/awizemann/harness/blob/main/standards/AUDIT_CHECKLIST.md). Pay particular attention to standard 03 (subprocess invocation rules) and standard 04 (error handling).
...
- `XcodeBuilder` — TBD
- `SimulatorDriver` — TBD
- `ClaudeClient` / `OpenAIClient` / `GeminiClient` — see [Claude-Client](Claude-Client). Each conforms to `LLMClient`; wire-format details live in the per-client file. Tool-name → typed `ToolCall` decode is shared via `LLMShared`. Provider dispatch is `LLMClientFactory.client(for:keychain:)`.
- `RunLogger` — TBD
- `RunHistoryStore` — TBD
- `KeychainStore` — TBD
