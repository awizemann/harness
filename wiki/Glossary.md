...
### Run
A single end-to-end agent session against one app build for one goal + persona pair. Has a UUID, a directory under `~/Library/Application Support/Harness/runs/<id>/`, and exactly one final `Verdict`. Bounded by `run_started` and `run_completed` rows in `events.jsonl`.

### Step
One iteration of the agent loop. Comprises: a fresh screenshot, one Claude call, one tool call, the tool's execution, zero or more friction events, and a `step_completed` row. Steps are 1-indexed within a run; numbers are gap-free and monotonic.

### Action
The thing the agent does on a step. Concretely, the typed payload behind a `tool_call`: a tap with coordinates, a swipe with endpoints, a typed string, a button press, etc. See `wiki/Tool-Schema.md`.

### Tool call
A model-emitted invocation of one Action (or `note_friction` / `mark_goal_done`). Carries the tool name, the typed input, and the agent's `observation` + `intent` reasoning fields.
...
### Goal
The plain-language description of what the user (the human running Harness) wants the agent to attempt. Stored on the run as a single text blob; never parsed or normalized.

### Persona
The plain-language description of who the agent is pretending to be. Stored on the run alongside the goal. Concatenated into the system prompt at `{{PERSONA}}`. Default personas in `docs/PROMPTS/persona-defaults.md`.
