...
A feature is a self-contained MVVM-F module under `Harness/Features/<Name>/`. Per [`../standards/01-architecture.md`](https://github.com/awizemann/harness/blob/main/standards/01-architecture.md), it owns its own Models, ViewModels, Views and never imports a sibling feature.
...
Read [`Design-System.md`](Design-System) for the full primitive table. Hard rules:
...
If the feature introduces a new pattern or surface, **add a row** to [Core-Services](Core-Services) (if a service was added) and update this page's "real examples" section below.

### 8. Audit

Run [`../standards/AUDIT_CHECKLIST.md`](https://github.com/awizemann/harness/blob/main/standards/AUDIT_CHECKLIST.md) over the diff before requesting review.

---

## Real examples

- `GoalInput/` — **"Compose Run" form**, post-redesign. Section header bar with `New Run / <Application name> · <scheme>` breadcrumb + preflight `Pill`. Body sections in order: run-name pill row (auto-name placeholder + `auto` badge when blank), hero heading, Simulator picker, Persona picker (with blurb), Source picker (`SegmentedToggle<RunSource>` action/chain + matching picker + inline ordered chain preview that flags broken steps with `FrictionTag(.deadEnd)`), Run-mode strip (two equal `ModeCell`s with radio dot + subtitle + `KbdKey` shortcut hints; `INHERITS APP` badge while unmodified), Advanced disclosure (Model `SegmentedToggle<AgentModel>` + Step-budget `Stepper`, each with `INHERITS APP` badges; collapsed-state header shows model + budget as `Pill`s). Sticky footer pins preflight status + Start Run (`AccentButtonStyle` + ⌘↵). Goal text always comes from a saved Action (or per-leg from a Chain) — there's no free-form goal textarea, no example chips, no context strip, no persona avatar hero. The Library surfaces (Applications / Personas / Actions) own that state. VM hydrates Personas / Actions / Chains via `runHistory.personas() / actions() / actionChains()` and synthesizes a `RunRequest` carrying a `RunPayload.singleAction` or `.chain`; hand-off through `AppContainer.stagePendingRun(_:)`.
...
For the agent-loop primer that ties the run lifecycle together, see [Agent-Loop.md](Agent-Loop).
