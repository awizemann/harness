...
- `GoalInput/` — **Phase E rewrote this** as the "Compose Run" form. The view gates on `coordinator.selectedApplicationID != nil`; when an Application is active, it shows the recap + run-name field + Persona picker + Source toggle (`SegmentedToggle<RunSource>`) + matching Action / Chain picker + an "Override defaults" disclosure. The VM hydrates Personas / Actions / Chains via `runHistory.personas() / actions() / actionChains()` and synthesizes a `RunRequest` carrying a `RunPayload.singleAction` or `.chain`. Hand-off still goes through `AppContainer.stagePendingRun(_:)` — only the value type changed (`GoalRequest` → `RunRequest`, with the old name kept as a typealias for back-compat).
