...
Harness's visual language is the **HarnessDesign** source folder at [`../HarnessDesign/`](https://github.com/awizemann/harness/blob/main/HarnessDesign/), included directly in the main app target. The full canonical guidance is in [`../standards/05-design-system.md`](https://github.com/awizemann/harness/blob/main/standards/05-design-system.md). This page is the index — token + primitive names that match the actual code.
...
Every primitive ships with a `#Preview`; the "Used by" column is the production wiring as of Phase 4. If you add a new feature view, prefer reusing a primitive over re-rolling one — the design-token discipline section in [`Adding-a-Feature.md`](Adding-a-Feature) lists the do/don'ts.
...
| `SimulatorMirrorView` | `HarnessDesign/Primitives/SimulatorMirrorView.swift` | RunSession (live mirror with last-tap dot) — iOS / macOS platforms |
| `WebMirrorView` | `HarnessDesign/Primitives/WebMirrorView.swift` | RunSession + RunReplay for web platform — flat panel with browser chrome (URL pill, lock glyph, back/forward/refresh affordances) instead of a device bezel; lets a 1280×1600 viewport fill the full middle column. |
...
See [`../standards/05-design-system.md §10`](https://github.com/awizemann/harness/blob/main/standards/05-design-system.md). Briefly: edit `HarnessDesign/`, update affected feature views, update this index page.

## Cross-references

- [`../standards/05-design-system.md`](https://github.com/awizemann/harness/blob/main/standards/05-design-system.md) — canonical guidance.
- [`../design-prompt.md`](https://github.com/awizemann/harness/blob/main/design-prompt.md) — historical artifact: the brief that produced HarnessDesign.

---

_Last updated: 2026-05-07 — added `WebMirrorView` primitive (web runs now use a flat browser chrome, not a device bezel). Out-of-scope follow-ups tracked in [`docs/DESIGN_BACKLOG.md`](https://github.com/awizemann/harness/blob/main/docs/DESIGN_BACKLOG.md)._
